...
async function requireQueryable(label, promise, result) {
  ...
  if (error) result.fail(`${label} query failed`, error.message);
    ...
    result.pass(
      `${label} is queryable${Array.isArray(data) ? ` (${data.length} row(s))` : ""}`,
    );
  return data ?? [];
}
...
if (snapshotError || !snapshot) {
  ...
  expectEqual(
    result,
    "all V1.1 business tables have RLS",
    snapshot.schema?.missing_rls_count,
    0,
  );
  expectEqual(
    result,
    "all required V1.1 transaction functions exist",
    snapshot.schema?.missing_function_count,
    0,
  );
  expectEqual(
    result,
  ...
  if (!purchase.purchase_order_count) {
    const message =
      "authoritative PI has not been committed to this environment";
    if (requirePiImport) result.fail(message);
    else result.warn(`${message}; set REQUIRE_PI_IMPORT=true after commit`);
  } else {
    expectEqual(
      result,
      "authoritative PI created one purchase order",
      purchase.purchase_order_count,
      1,
    );
    expectEqual(
      result,
      "authoritative PI line count is 119",
      purchase.line_count,
      119,
    );
    expectEqual(
      result,
      "authoritative PI has 119 unique Part Numbers",
      purchase.unique_part_number_count,
      119,
    );
    expectEqual(
      result,
      "authoritative PI quantity is 706",
      purchase.quantity,
      706,
    );
    expectEqual(
      result,
      "authoritative PI subtotal is exact",
      purchase.subtotal_minor,
      6313780,
    );
    expectEqual(
      result,
      "authoritative PI discount allocation is exact",
      purchase.discount_minor,
      13780,
    );
    expectEqual(
      result,
      "authoritative PI cash cost is exact",
      purchase.cash_cost_minor,
      6300000,
    );
    expectEqual(
      result,
      "authoritative PI final total is exact",
      purchase.final_total_minor,
      6300000,
    );
    expectEqual(
      result,
      "all imported products remain at initial release gates",
      purchase.gate_mismatch_count,
      0,
    );
    expectEqual(
      result,
      "no imported product is publicly visible",
      purchase.public_product_count,
      0,
    );
    expectEqual(
      result,
      "imported purchase stock remains unavailable before receipt",
      purchase.available_stock,
      0,
    );
  }
}
...
await requireQueryable(
  ...
  supabase
    ...
    .select("id, contract_number, status, source_import_run_id")
    .limit(1),
  result,
);
await requireQueryable(
...
const { data: profiles, error: profilesError } = await supabase
  ...
  .select("id, role");
if (profilesError)
  result.fail("user_profiles query failed", profilesError.message);
else {
  ...
  for (const role of ["trade", "warehouse", "admin"]) {
    if (roles.has(role)) result.pass(`${role} test profile exists`);
      ...
      result.warn(
        `${role} test profile is missing; create it before AAL2 staging smoke tests`,
      );
  }
}
