...
export function TradeAccountApplicationForm() {
  ...
  useUnsavedChanges(Object.values(form).some(Boolean) || privacyConsent || tradeTermsConsent || status === "submitting");
  ...
  async function submitApplication(event: FormEvent) {
    ...
    setStatus("submitting"); setMessage("Submitting your application...");
    try {
    ...
    setStatus("success"); setMessage(`Application ${body.application.id} has been received.`); setForm(emptyForm); setPrivacyConsent(false); setTradeTermsConsent(false); setTurnstileToken("");
      ...
      setStatus("error"); setMessage("The application result could not be confirmed. Your details are retained. Contact account support before submitting again.");
    } finally { pending.current = false; }
  }

  return <form className="account-form" onSubmit={submitApplication}>
    <fieldset disabled={!hydrated || status === "submitting"} style={{ border: 0, margin: 0, padding: 0, minWidth: 0 }}>
    ...
    {!hydrated && <p className="form-message" role="status">Preparing secure form. JavaScript is required to continue.</p>}
  </form>;
}
