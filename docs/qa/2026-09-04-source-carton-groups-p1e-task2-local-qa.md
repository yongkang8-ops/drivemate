...
| 箱组、收货 API 与 repository 聚焦 Vitest | 10 passed |
| 全量 Vitest（最终提交前） | 56 files; 265 passed; 1 skipped |
| TypeScript | pass |
...
- 权限核对：三个公开 wrapper 仅 `service_role` 可调用；六个内部/校验函数对 `service_role` 均为 false。
